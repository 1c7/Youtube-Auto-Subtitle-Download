// ==UserScript==
// @name        Youtube English(auto-generated) Subtitle Downloader v1
// @description  download youtube AUTO subtitle (English-only)
...
// Some video are special
// like this one: https://www.youtube.com/watch?v=cDHhrqzcobk
// it got English(auto-generated) and German(auto-generated)
// 2 type of auto-generated subtitle
// that's why I create this base on "Youtube Auto Subtitle Downloader v12" (which able to download other language's auto subtitle)
...
function init(){
    // Put button on page
    $('#eow-title').append('<a id="YT_en_auto"> // Get English (auto-generated) Subtitle | 下载(英文)自动字幕 // </a>');

    // Style
    $('#YT_en_auto').addClass('start yt-uix-button yt-uix-button-text yt-uix-tooltip'); // 样式是 Youtube 自带的.
    $("#YT_en_auto").css('margin-top','2px')
        .css('margin-left','4px')
    ...
    if (version > 52){
        document.getElementById('YT_en_auto').setAttribute(
            'download',
            '(auto)' + TITLE + '.srt'
        );
        document.getElementById('YT_en_auto').setAttribute(
...
function get_subtitle(){
    ...
    $.ajax({
        ...
        async: false,
        error: function(r){
          console.log("English Auto Subtitle Download: Ajax Error");
        },
        success: function(r) {
...
function noAutoSubtitleHint(){
  $("#YT_en_auto").text("No English Auto Subtitle | 没有(英文)自动字幕");
  $("#YT_en_auto").css("background-color","rgb(0, 0, 0)").css('border','1px solid rgb(0, 0, 0)');
}
