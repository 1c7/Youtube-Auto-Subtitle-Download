// ==UserScript==
// @name        Japanese Youtube Auto Subtitle Downloader v1
// @description  download youtube Japanese AUTO subtitle
...
function init(){
    //  加按钮
    $("#eow-title").append('<a id="YT_auto_jp"> [Download] Japanese Auto Subtitle </a>');

    //  调样式
    $("#YT_auto_jp").addClass('start yt-uix-button yt-uix-button-text yt-uix-tooltip'); // 样式是 Youtube 自带的.
    $("#YT_auto_jp").css('margin-top','2px')
        .css('margin-left','4px')
    ...
    // 鼠标悬浮时改背景颜色;
    $("#YT_auto_jp").hover(function() {
        $(this).css("background-color","rgb(0, 163, 80)")
            .css("border","1px solid rgb(0, 183, 90)");
    });
    $("#YT_auto_jp").mouseout(function() {
    ...
    if (chrome_version > 52){
        document.getElementById('YT_auto_jp').setAttribute(
            'download',
            '(auto)' + TITLE + '.srt'
        );
        document.getElementById('YT_auto_jp').setAttribute(
...
function get_subtitle(){
    ...
    if (!TTS_URL){
        $("#YT_auto_jp").text("No Japanese Auto Subtitle");
        return false;
    }

    var xml = TTS_URL + "&type=track" + "&lang=ja" + "&name" + "&kind=asr";
    console.log(xml);
    // 拼xml字幕链接地址
