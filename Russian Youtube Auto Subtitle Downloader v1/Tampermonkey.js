// ==UserScript==
// @name        Russian Youtube Auto Subtitle Downloader v1
// @description  download youtube Russian AUTO subtitle
...
function init(){
    //  加按钮
    $("#eow-title").append('<a id="YT_auto_russian"> [Download] Russian Auto Subtitle </a>');

    //  调样式
    $("#YT_auto_russian").addClass('start yt-uix-button yt-uix-button-text yt-uix-tooltip'); // 样式是 Youtube 自带的.
    $("#YT_auto_russian").css('margin-top','2px')
        .css('margin-left','4px')
    ...
    if (chrome_version > 52){
        ...
        $("#YT_auto_russian").click(function(){
...
function get_subtitle(){
    ...
    if (!TTS_URL){
        $("#YT_auto_russian").text("No Russian Auto Subtitle");
        return false;
    }

    var xml = TTS_URL + "&type=track" + "&lang=ru" + "&name" + "&kind=asr";
    // 拼xml字幕链接地址
